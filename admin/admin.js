...
function updateImagePreview() { const url = safeImageUrl($('#image-url').value); const preview = $('#image-preview'); const image = $('#image-preview-image'); preview.hidden = !url; if (url) image.src = url; }
function fillForm(article) { if (!article) return; state.editingId = article._id; $('#article-id').value = article._id; $('#title').value = article.title; $('#summary').value = article.summary; $('#category').value = article.category; $('#author').value = article.author || ''; $('#image-url').value = article.imageUrl || ''; $('#published').checked = article.published; $('#form-title').textContent = 'Editar notícia'; updateImagePreview(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
...
$('#image-url').addEventListener('input', updateImagePreview); $('#clear-form').addEventListener('click', () => { clearForm(); updateImagePreview(); }); $('#refresh-button').addEventListener('click', loadArticles); $('#logout-button').addEventListener('click', () => { sessionStorage.removeItem('pva_admin_credentials'); state.credentials = ''; setAuthenticated(false); });
